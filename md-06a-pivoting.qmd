...
2.  What's the `tidyverse` Package? Describe in maximum two sentences below.
    ...
    It's a set of packages that can be used together and enable somewhat of a standardization of data processing. It's what we've been using in the lecture.
...
waste_data_long <- waste_data_fct |> 
  pivot_longer(
    cols = c(food_organic_waste, yard_garden_green_waste),
    names_to = "waste_category",
    values_to = "percent"
  ) |> 
  drop_na(percent)

#| label: tbl-waste_data_long
waste_data_long |> 
  kable()
```
...
1.  Starting with the `waste_data_long_mean` object, execute the code and observe the output in the Console. Would you publish this table in this format in a report?
    ...
    No, this is a good format for data processing, but not necessarily for direct viewing.
...
2.  Execute the code and observe the output in the Console. Would you publish this table in a report in this format?
    ...
    Yes, it gives a significantly better overview with increased readability.
